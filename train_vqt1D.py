...
def train_vqt(
    ...
    tf = transforms.Compose(
        [transforms.Grayscale(), transforms.ToTensor(), transforms.Normalize((0.5,), (1.0))]
    )
    ...
    for i in range(n_epoch):
        ...
        ddpm.eval()
        with torch.no_grad():
            xh = ddpm.sample(8, (1, 32, 32), device)
            xset = torch.cat([xh, x[:8]], dim=0)
            grid = make_grid(xset, normalize=True, value_range=(-1, 1), nrow=4)
            save_image(grid, f"./vqt_result1D/ddpm_sample_vqt{i}.png")

            # save model
            torch.save(ddpm.state_dict(), f"./ddpm_vqt.pth")
